...
% Receiver(s) (quantized locations in a region)
...
% === LIS (one of the BS *locations* - considered passive here of course) ===
...
disp('==========Generating Transmitter-IRS Full Channel============');
% ------------------ DeepMIMO "Ut" Dataset Generation ----------------- %
params.active_user_first=Ut_row;
params.active_user_last=Ut_row;   % Only one active user (but where is Ut_element to fully specify the user??)
DeepMIMO_dataset=DeepMIMO_generator(params); % Generator function generates data for entire rows
...
for pp = 1:1:numel(Ur_rows_grid)-1
    ...
    params.active_user_first=Ur_rows_grid(pp);
    params.active_user_last=Ur_rows_grid(pp+1)-1;
    [DeepMIMO_dataset,params]=DeepMIMO_generator(params); % Why generating again and not sampling from the beginning?
...
%% DL Beamforming (Using Deep Learning)
disp('======================DL BEAMFORMING=========================');
